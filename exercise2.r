...
convert_type_to_int <- function(data, column, string) {
  converted_frame <- data |> 
    mutate(column = as.integer(str_remove_all(column, pattern = string)))
...
pop_density <- load_formatted_frame("population_density_2019.csv")
...
population_density_correct_types <- convert_type_to_int(pop_density_single_code, pop_density_single_code$country_code, "CC")
locations_correct_types <- convert_type_to_int(locations, locations$location_id, '"')
