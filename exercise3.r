...
join_frames <- function(data1, data2, first_joincol, second_joincol) {
  colnames(data2)[colnames(data2) == second_joincol] <- first_joincol
  
  joined_data <- left_join(data1, data2, by = first_joincol) |> 
    select(sdg_region_name, population_density)
